...
Run only during active user turns. Never create or use automations, scheduled tasks, heartbeats, cron, timers, delayed wakeups, or background watchers. After a successful direct handoff, the sender may call app-task `wait_threads` exactly once with `timeoutMs: 0` for the exact recipient only to confirm its state. Never pass a wait cursor, wait for completion, or call again for that handoff. Later progress arrives through the return baton or a new user turn. If CI or another external condition is still pending, report it and wait for a new user message.
...
2. Verify the required app-task and native-reviewer tools are callable. Resolve the initiating task's exact saved `projectId`; every persistent planner, manager, or worker task must use that same recorded project ID, including reused tasks. Recommend an available model and supported reasoning effort for the planner, with a short reason. Use native input to settle planner staffing and optional Fable review, reusing choices already established. Report an unavailable capability or project mismatch before creating or messaging tasks; app tasks and native children are distinct.
3. After confirmation, require a clean GitHub-backed saved local checkout. Fix the role topology: the calling task is the coordinator unless the user explicitly makes it the manager; never create a second manager. Create `docs/baton-pass/<session-id>/`, add `/docs/baton-pass/` only to the checkout's local `.git/info/exclude`, and write `brief.md` plus a small `status.md` containing the session, repository, recorded project ID, topology, current holder, and role task IDs.
4. Create one fresh planner app task with the recorded `projectId`, `environment: {type: "local"}`, the confirmed model and effort, and a prompt containing the session path, task brief, and its exact next handoff from [Handoffs](#handoffs). Do not create manager or worker yet.
...
3. Choose any currently available model and supported reasoning effort for manager and worker. Explain what each assignment needs and why that model/effort fits. Role names do not imply model families.
...
5. Otherwise ask the user to approve or revise the plan and staffing together. After approval, create fresh manager and worker app tasks with the same recorded project ID and `environment: {type: "local"}` using the approved combinations. Give each an inert setup prompt naming its role, session path, and exact next handoff.
...
2. Set worker as holder in `status.md`, send one concise handoff, and take only the single immediate confirmation snapshot allowed above. If that snapshot already shows completion or needed attention, continue at step 3; otherwise stop and await the return baton or a new user turn.
...
5. Ask the original planner, or the accepted plan's advisor, for plan-fidelity review. Only manager acceptance and its `PASS` advance the candidate to final review; adjudicate findings against accepted scope before assigning corrections.
